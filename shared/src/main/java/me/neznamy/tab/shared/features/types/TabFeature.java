...
public abstract class TabFeature {
    ...
    protected TabFeature(@NonNull String configSection) {
        List<String> disabledServers = TAB.getInstance().getConfig().getStringList(configSection + ".disable-in-servers");
        List<String> disabledWorlds = TAB.getInstance().getConfig().getStringList(configSection + ".disable-in-worlds");
