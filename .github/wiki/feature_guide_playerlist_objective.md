...
To get started, run `/tab dump <any player>` (this feature doesn't contain per-player content in dump output) and open the generated link. Scroll down to `features` -> `PlayerlistObjective` and check the content:
* If it says `PlayerlistObjective: Feature is disabled`, it means you disabled the feature. Enable it by setting
  ```
  playerlist-objective:
    enabled: true
  ```
...
* If a player cannot see playerlist objective on anyone, check the table of players and make sure it's not because `Disabled with condition` is `true`.
